...
Optimize across recruiter discoverability, on-visit credibility, and differentiation. Tailor to target role context when available.
...
- `coaching_state.md` — Load Profile (target role, seniority band), Resume Analysis (positioning strengths, concerns, story seeds), Storybank (earned secrets for About/Experience rewrites), Active Coaching Strategy (if Differentiation is bottleneck, emphasize in LinkedIn too), Positioning Statement (use as consistency reference for headline/about rewrite), Resume Optimization (cross-reference for consistency check)
...
### How LinkedIn Actually Works (Reference Knowledge)
...
**Step 3: Section-by-Section Audit (9 sections for Standard/Deep)**
...
### Step 4: Positioning Consistency Check (Standard + Deep)
...
### Step 5: Content Strategy (Standard + Deep, Timeline-Aware)

If Profile has an interview timeline, calibrate the content recommendation:
...
- **6+ weeks or early-stage search**: Full content strategy below.
...
- If LinkedIn Analysis "Weak" or "Needs Work" and no `prep` scheduled ≤2 weeks → `linkedin` (Standard) for full audit
- If interview scheduled ≤2 weeks and LinkedIn still needs work → `hype` (prioritize interview prep)
- If no interview scheduled ≤2 weeks → `stories` (earned secrets feed into LinkedIn rewrites)
- Default → `stories` or `prep [company]`
