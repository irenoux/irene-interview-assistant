...
Full simulated interview (4-6 questions in sequence) with holistic feedback on the full arc. Produces a Hire Signal and comprehensive debrief.

## Input Requirements

- Interview format (behavioral screen, system design, panel, etc.)
- Company and role context (ideally)
- Candidate's self-assessment before the mock starts

## Optional Inputs

- Specific company/role (for realistic contextualization)
- Known interviewer personas (for panel mocks)

## State Files Needed

- `coaching_state.md` (if exists — for prep data, storybank, company context, format preferences)
- Interview Loops data (for realistic mock calibration)

## Workflow

### Step 1: Setup & Format Confirmation

1. Ask for format (behavioral screen, deep behavioral, panel, bar raiser, system design/case study, technical+behavioral mix, etc.). **See format taxonomy in `references/commands/prep.md`**.

2. **For system design/case study and technical+behavioral mix**: Check Interview Loops for saved format data from `prep`. If format data exists for this company+round, use it. If no format data exists, run the Format Discovery Protocol (see `references/commands/prep.md`) and save the result to Interview Loops.

3. Ask for company/role context (or use existing prep data).

4. **Calibrate difficulty to the candidate's progression stage.** Check Drill Progression in `coaching_state.md` — if the candidate is at Stage 2 (pushback), the mock should be calibrated to push on credibility. If they're at Stage 6+ (panel/stress), they can handle maximum intensity. Don't run a maximum-difficulty mock for a candidate who hasn't cleared basic constraint drills.

5. **Calibrate tone to the target company.** A mock for a FAANG final round should feel very different from a Series A startup first call:
   - Large tech companies: structured, higher bar on specificity and metrics, interviewers often follow rubrics
   - Startups: conversational, care about adaptability and scrappiness, may go off-script
   - Consulting/finance: case-study oriented, precision matters, presentation polish expected
   - If prep data exists for this company, use the culture read and format analysis to shape the mock's feel.

6. Set interviewer persona based on format. For panel, deploy 2-3 distinct interviewer archetypes from `references/role-drills.md`.

### Step 2: Pre-Mock Self-Assessment

Before starting the mock, ask the candidate for their baseline self-assessment:
- "Before we start — how confident are you going in? Strong Hire, Hire, Mixed, or No Hire?"

This anchors their expectations. Record their response to compare at the end.

### Step 3: Execute the Mock
...
7. **Track throughout**: story diversity (did they use the same story twice?), energy trajectory, answer length distribution, time management.

### Step 4: Format-Specific Simulation
...
**Before starting, check Interview Loops for saved format data from `prep`.** If format data exists, use it. If not, run the Format Discovery Protocol and save the result. If the candidate has described their specific format, simulate THAT. If neither data source exists, default to a verbal walkthrough format (the most coachable variant) and say so.
...
4. For the technical discussion portions, follow the System Design simulation guidelines above.
...
**What to track**:
- **Mode-switching speed and quality**: How quickly and cleanly does the candidate shift? Do they fumble transitions or handle them fluidly?
...
- **Integration quality**: Do their behavioral stories and technical discussions reinforce each other, or feel disconnected?
...
- **Which mode is stronger**: Is there a visible gap between their behavioral and technical performance? This is critical coaching data.

### Step 5: Post-Mock Self-Assessment
...
### Step 6: Redo Mechanism

After delivering per-question feedback in the debrief, offer one redo opportunity for the weakest answer: "Your answer to Q[N] had the most room for improvement. Want to try that one again right now? I'll re-ask the question and give you fresh feedback — it's the fastest way to lock in the improvement."

If the candidate takes the redo, re-ask the question, score it independently, and show the before/after comparison. Only offer one redo per mock — this isn't a full practice session.
...
- Strongest moment: [what worked in this answer]
- Missed opportunity: [what could have made this stronger]

[...repeat for each unit]

## Holistic Patterns (things only visible across the full interview)
- Repeated crutch phrases: [any overused words or phrases]
- Topics avoided: [what the candidate steered away from]
- Questions that caused visible hesitation: [which questions were uncomfortable]
- Best moment of the interview: [the single strongest answer or exchange]
- Worst moment and recovery quality: [the weakest answer + how they bounced back]

## Signal Reading Notes
- Questions where follow-up indicated interest (positive signal): [which questions got deep follow-ups]
- Questions where interviewer moved on quickly (negative signal): [which questions got short follow-ups]
- Questions where interviewer redirected (answer wasn't landing): [which questions needed redirection]
...
1. [specific change, rooted in the debrief data]
2. [specific change]
3. [specific change]

**Recommended next**: `[command]` — [reason based on the debrief findings]. **Alternatives**: `mock [same format]`, `practice [specific drill]`, `practice technical`, `analyze`
```
...
**Connect to signal-reading.** The monologue should explain the interviewer behaviors the candidate would have seen.
...
After the mock debrief, update `coaching_state.md`:
...
3. **Update Active Coaching Strategy** if the mock reveals new patterns or confirms/contradicts the current strategy. Preserve Previous approaches when updating.
...
- **`mock [format]` explicit command** → Execute the mock for that format
- **`mock` with no format specified** → Ask for format, then execute
- **"I want to do a full mock interview"** → Ask for format, then execute

## Multi-Step Intent

"Practice a full interview at [company]" → `mock [format]` (if prep exists) or `prep [company]` (if not) → `hype` (if ≤48h until real interview)
