...
Lightweight company research for evaluating fit and deciding whether to apply. Alternative to `prep` when interview is not yet scheduled.

## Input Requirements

- Company name
- Candidate's target role type (if not already in coaching state)
- Optional: depth level preference (Quick Scan, Standard, Deep Dive)

## State Files Needed

- `coaching_state.md` (if exists — for candidate profile context)
- Resume Analysis (for fit assessment comparison)

## Workflow

### Step 1: Gather Company Name & Role Context

Ask for company name and target role type. If candidate already in coaching state, reference their profile seniority band and target roles. If new candidate, infer from what they tell you.
...
- Candidate has an interview scheduled at this company
- Candidate explicitly asks for comprehensive intelligence
- Company is in their top 3 targets

### Step 3: Structured Search Protocol
...
### Step 4: Claim Verification Protocol

Every company-specific claim must map to a source tier:

- **Tier 1 — Verified**: Information directly retrieved from the company's own website, careers page, blog, or from job description/candidate-provided context. Cite the source.
...
- Never synthesize multiple uncertain sources into a confident claim.

### Step 5: Fit Assessment (Role-Fit Assessment Module)
...
After research completes, save a lightweight entry to `coaching_state.md` Interview Loops:
...
**Recommended next**: `prep [company]` — build a full prep brief now that you have the research foundation. **Alternatives**: `research [another company]`, `stories`, `decode [JD]`

---

## Mode Detection

- **`research [company]` explicit command** → Execute immediately
- **"I want to know about [company]" / "Should I apply to [company]?"** → Route to `research`
- **Building a target company list** → Suggest running `research` multiple times

## Multi-Step Intent

"Build a target list" / "Which of these companies should I apply to?" → Run `research` for each company in sequence, then suggest `decode` for batch triage of JDs.
