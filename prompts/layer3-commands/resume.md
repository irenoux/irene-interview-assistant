...
Improve resume performance across ATS, recruiter scan, bullet quality, seniority signaling, keyword coverage, and consistency with other surfaces.
...
- `coaching_state.md` — Load Profile (target role, seniority band), Resume Analysis (to avoid re-deriving what's already been assessed), Storybank (for earned secrets and quantified outcomes for bullet rewrites), Active Coaching Strategy (if Differentiation is the gap, emphasize in resume), LinkedIn Analysis (if exists — for cross-surface consistency at Deep level), Interview Loops (for JDs — enables targeted optimization), Positioning Statement (if exists — use as consistency reference for summary rewrite)
...
### How Resumes Actually Work (Reference Knowledge)
...
- Verb seniority match: IC verbs for IC roles, manager verbs for manager roles, director verbs for director roles
...
### Experience — [All roles listed]
[full bullet rewrites for every role]
...
- Targeting checklist: [what to customize per application]
...
- If LinkedIn Analysis exists → `linkedin` (cross-surface consistency)
- If interview scheduled ≤2 weeks → `prep [company]` (time-sensitive)
- If Storybank empty → `stories` (feed into resume rewrites)
- Default → `linkedin` or `prep [company]`
