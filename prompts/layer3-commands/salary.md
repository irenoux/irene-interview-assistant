...
Coach the candidate through compensation conversations at application form, recruiter screen, mid-process, and pre-offer stages. Research guidance (candidate-driven), range construction, stage-specific scripts, total comp education, and startup equity evaluation.

## Purpose

Build compensation strategy before an offer arrives (pre-offer stage). Covers research guidance, range construction, and scripts for each comp conversation stage. Post-offer negotiations are handled by `negotiate` command.
...
- `coaching_state.md` — Load Profile (target role, seniority, location), Interview Loops (active companies, comp data for each if previously shared), Resume Analysis (current level), JD Analysis (comp data from JD if decode was run), Comp Strategy (if salary was run previously — build on it, don't restart)
...
- **Salary transparency postings** — Many jurisdictions now require salary ranges (Colorado, NYC, California, Washington state, etc.). Most reliable when available.
...
- **Equity** — RSUs (public company, liquid), ISOs/NSOs (startup, illiquid), shares/units (private, speculative). Valuation, vesting schedule (standard: 4-year with 1-year cliff), dilution risk, and liquidity timeline matter as much as grant size.
...
- **Remote roles at HQ-based companies**: Many companies pay based on the candidate's location (cost-of-living adjustment), not HQ. Ask: "Is this role remote? If so, does the company adjust comp by location?" If yes, the range needs geographic calibration.
- **Relocation**: If the candidate is relocating, research the target location's market rate, not their current one.
- **Hybrid vs. fully remote**: Some companies offer different comp bands for hybrid vs. fully remote.
- **Cost-of-living resources**: Point candidates to geographic salary comparison tools alongside Levels.fyi/Glassdoor.
...
1. **What stage is the company?** Pre-seed/seed (highest risk, highest potential upside), Series A-B (validated product, still significant risk), Series C+ (lower risk, lower upside multiplier), Pre-IPO (most predictable, but check lockup periods).
...
- Framing language: [how to position a comp increase — value-based, not cost-based]
...
After running `salary`, save to coaching_state.md:
...
- If interview within 48 hours → `prep [company]` (interview prep takes priority)
- If comp conversation happening imminently (within 24-48h) → ensure scripts are ready, then move to interview prep
- If offer arrives → redirect to `negotiate`
- Default → `prep [company]` or active interview prep
