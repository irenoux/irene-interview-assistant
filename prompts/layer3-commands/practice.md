...
Practice drill menu and progression system. Builds interview skills through constraint-based, targeted drills (8 progression stages + standalone retrieval).

## Input Requirements

- Drill type (ladder, pushback, pivot, gap, role, panel, stress, technical, or retrieval)
- Optional: target company/role context (for question tailoring)

## State Files Needed

- `coaching_state.md` (for Drill Progression status, storybank, target roles, previous scores)

## Workflow

### Step 1: Show Practice Menu

Display the full menu with current progression status:

```
Practice Menu

PROGRESSION LADDER (gated by stage):
...
OPTIONAL (not gated):
8) practice technical     — Thinking out loud, clarification-seeking, tradeoff articulation (system design/mixed format only)

STANDALONE (not gated):
•  practice retrieval     — Rapid-fire question-to-story matching under time pressure (requires 8+ stories)
```

### Step 2: Check Progression Status

Consult `coaching_state.md` Drill Progression section. Determine which drills are available and which require gating.
...
### Step 3: Question Tailoring

Don't throw generic drill questions. Before each practice session, pull from:

- **Target companies and roles** (from `coaching_state.md`)
- **Known weak spots** (from previous analyses or practice rounds)
- **Storybank gaps** where no strong story exists — when a question hits a known gap, coach using the appropriate gap response pattern from the Gap-Handling Module in `references/cross-cutting.md`. Map pattern selection to storybank scores: strength 2 → Adjacent Bridge, strength 1 → Reframe to Strength or Growth Narrative, no story → Hypothetical with Self-Awareness.
- **Specific competencies** the candidate's target JDs emphasize
...
- Transition: "Good, you're warmed up. From here on I'll score each round."
...
### Step 5: Round Protocol (every drill round)

1. State round objective (what this drill targets).
...
8a. **Round Challenge (Level 5, rounds 3+ only)**: Apply one Challenge Protocol lens per round, rotated: Assumption → Blind Spot → Pre-Mortem → Devil's Advocate → cycle back. Keep to 1-2 sentences. At Levels 1-4: skip.
9. Set one specific change for next round.

### Step 6: Round Output Schema

```markdown
## Round Debrief
- Drill: [drill type]
- Objective: [what this round targets]
- Candidate Self-Assessment: [their rating before feedback]
...
- Candidate rated themselves: __ [their self-assessment]
- Coach scored: __ [your independent score]
- Calibration gap (if any): [delta interpretation]

## Interviewer's Read
[1-2 key moments from this round, told from the interviewer's perspective. At Level 5, rounds 3+: expand to mini Inner Monologue (3-4 sentences)]

## Challenge Note (Level 5, rounds 3+ only)
[One lens, 1-2 sentences]

## Next Round Adjustment
- Try this single change: [one specific change to work on]
...
Keep it to 1-2 moments per round — practice rounds are short. Pick the moments with the highest teaching value.

**Always include at least one positive moment** — what would have genuinely impressed an interviewer.
...
**Connect to the scoring.** The Interviewer's Read should make the scorecard *make sense*. If Structure scored a 2, the monologue should show what that felt like from the other side of the table.

**Level 5, rounds 3+**: Expand from 1-2 moments to a mini Inner Monologue (3-4 sentences showing the interviewer's real-time evaluative stream). Show what the answer felt like from the other side, including positive reactions, doubt, and pivot points. At Levels 1-4, keep to the standard 1-2 moments.

### Step 7: Revisit Queue
...
### Step 8: End-of-Session Coaching State Updates
...
## Special Drill Protocols

### `practice technical` — Session Protocol

When the candidate runs `practice technical`, don't just throw all drills at them. Run a structured session:
...
2. **Check Format Discovery data.** If the candidate has previously described their specific interview format (stored in Interview Loops or Profile), reference it: "You told me your system design round is a collaborative verbal walkthrough. I'll tailor the drills to that format."
...
3. **Select 1-2 drills per session.** Don't run all four — a 30-minute session covering Thinking Out Loud + Clarification-Seeking is better than a shallow pass through all four. Selection logic:
   ...
   - **If recent practice/analyze scores show weak tradeoff articulation**: Prioritize Tradeoff Articulation drill.
...
The stress drill is the final test before a real high-stakes interview.

**Session setup:**

1. **Gate check.** Confirm the candidate has completed Stages 1-5. If not, flag it: "The stress drill is designed for candidates who've built a solid foundation. Your current stage is [X]. Want to work on [prerequisite drill] first, or push ahead anyway?" Respect their choice.
...
Retrieval is a standalone drill — not gated by the progression ladder — because it's a storybank maintenance skill, not a core interview skill.
...
- **`practice [drill]` explicit command** → Execute the specified drill
- **`practice` with no drill specified** → Show the menu
- **"I want to practice [format]"** → Route to `practice [drill]` matching the format (e.g., "practice panel" for panel format)

## Multi-Step Intent

"I want to build up my interview skills" / "Get me ready to interview" (3+ weeks timeline) → Run through practice progression ladder in sequence (ladder → pushback → pivot → gap → role → panel → stress)
